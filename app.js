...
function render(){
  ...
  document.querySelector('#rows').innerHTML=f.map(x=>{
    ...
    return `<tr data-id="${DATA.indexOf(x)}"><td><span class="player">${x.player}</span><span class="sub">${x.team} · ${x.position||''}</span></td><td>${x.team} ${x.home?'vs':'@'} ${x.opponent}</td><td><span class="pill">${x.market_label}</span><span class="book">${x.bookmaker_label||x.bookmaker||''}</span></td><td>${fmt(x.line)}</td><td>${fmt(avg(l5))}</td><td>${fmt(avg(l10))}</td><td>${h.length?fmt(avg(h)):'—'}</td><td>${fmt(x.projection)}</td><td class="${edge>=0?'positive':'negative'}">${edge>=0?'+':''}${fmt(edge)}</td><td class="rate">${rate(l10,x.line)}%</td></tr>`;
...
function openModal(x){
  ...
  document.querySelector('#modalMarket').textContent=x.market_label.toUpperCase()+' · '+(x.bookmaker_label||x.bookmaker||'SPORTSBOOK').toUpperCase();
