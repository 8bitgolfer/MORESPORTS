...
## V3 stats source
Historical WNBA scoring data is now built from ESPN WNBA scoreboards and box scores. The updater caches completed-game box scores in `data/espn_boxscore_cache.json`, so later runs only request games that have not already been saved. No BALLDONTLIE key is needed.
